...
def folder2db(
        ...
        #folder2db(kb_names=args.kb_name, mode="recreate_vs", embed_model=args.embed_model)这是前面传过来的参数

        kb_names: List[str],
        mode: Literal["recreate_vs", "update_in_db", "increament"],
        vs_type: Literal["faiss", "milvus", "pg", "chromadb"] = "milvus",#DEFAULT_VS_TYPE = "faiss"
    ...
    def files2vs(kb_name: str, kb_files: List[KnowledgeFile]):
        for success, result in files2docs_in_thread_(kb_files,
            ...
            if success:
                ...
                print(f"正在将 {kb_name}/{filename} 添加到向量库，共包含{len(docs)}条文档")#这里的文档指的是文件划分为splliter之后的内容；
    ...
    for kb_name in kb_names:#vs_type = 'faiss'；kb_name = 'kb_name';embed_model = 'qwen-api'
        ...
        if mode == "recreate_vs":
            kb.clear_vs()
            kb.create_kb()
            a = list_files_from_folder(kb_name)#这个是获取到所有的文件名称
            kb_files = file_to_kbfile(kb_name, list_files_from_folder(kb_name))#这里就是很简单的获取到所有的knowledge类；
